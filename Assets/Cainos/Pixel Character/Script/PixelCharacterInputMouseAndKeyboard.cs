...
namespace Cainos.CustomizablePixelCharacter
{
    ...
    public class PixelCharacterInputMouseAndKeyboard : MonoBehaviour
    {
        ...
        private bool inputGhost;
        private bool inputInteract;
        private bool inputOption;
        ...
        private void Update()
        {
            ...
            else
            {
                ...
                inputGhost = Input.GetKeyDown(ghostKey);
                inputInteract = Input.GetKeyDown(interactKey);
                inputOption = Input.GetKeyDown(optionKey);
            ...
            if (controller)
            {
                ...
                if (inputJumpDown) controller.Jump();
                // controller.inputJumpDown = inputJumpDown;
                controller.inputAttack = inputAttack;
                controller.inputMelee = inputMelee;
                controller.inputLook = inputLook;
                controller.inputGhost = inputGhost;
                if (inputGhost) controller.GhostStart();
                controller.inputInteract = inputInteract;
                controller.inputTarget = inputTarget;
            }
        }

        public void InputKeyReset()
        {
            inputMove = Vector2.zero;
            inputRun = false;
            inputJump = false;

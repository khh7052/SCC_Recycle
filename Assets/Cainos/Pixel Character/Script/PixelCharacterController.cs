...
namespace Cainos.CustomizablePixelCharacter
{
    public class PixelCharacterController : MonoBehaviour
    {
        ...
        [Space]
        [FoldoutGroup("Movement")] public bool moveEnabled = true;
        [FoldoutGroup("Movement"), ShowIf("moveEnabled")] public float walkSpeedMax = 2.5f;                           // max walk speed
        [FoldoutGroup("Movement"), ShowIf("moveEnabled")] public float walkAcc = 10.0f;                               // walking acceleration
        [Space]
        [FoldoutGroup("Movement"), ShowIf("moveEnabled")] public float runSpeedMax = 5.0f;                            // max run speed
        [FoldoutGroup("Movement"), ShowIf("moveEnabled")] public float runAcc = 15.0f;                                // running acceleration
        [Space]
        [FoldoutGroup("Movement"), ShowIf("moveEnabled")] public float crouchSpeedMax = 1.0f;                         // max move speed while crouching
        [FoldoutGroup("Movement"), ShowIf("moveEnabled")] public float crouchAcc = 8.0f;                              // crouching acceleration
        [Space]
        [FoldoutGroup("Movement"), ShowIf("moveEnabled")] public float crawlSpeedMax = 1.0f;                          // max move speed while crawling
        [FoldoutGroup("Movement"), ShowIf("moveEnabled")] public float crawlAcc = 8.0f;                               // crawling acceleration
        [Space]
        [FoldoutGroup("Movement"), ShowIf("moveEnabled")] public float airSpeedMax = 2.0f;                            // max move speed while in air
        [FoldoutGroup("Movement"), ShowIf("moveEnabled")] public float airAcc = 8.0f;                                 // air acceleration
        [Space]
        [FoldoutGroup("Movement"), ShowIf("moveEnabled")] public float groundDrag = 10.0f;                            // braking acceleration (from movement to still) while on ground
        [FoldoutGroup("Movement"), ShowIf("moveEnabled")] public float airDrag = 0.75f;                               // braking acceleration (from movement to still) while in air
        [Space]
        [FoldoutGroup("Movement")] public bool jumpEnabled = true;
        [FoldoutGroup("Movement"), ShowIf("jumpEnabled")] public int maxJumpCount = 2;
        ...
        [Space]
        [FoldoutGroup("Movement")] public bool ghostEnabled = true;
        [FoldoutGroup("Movement"), ShowIf("ghostEnabled")] public float ghostHoildingTime = 3f; // 유체이탈 유지시간
        [FoldoutGroup("Movement"), ShowIf("ghostEnabled")] public float ghostCooldown = 1f; // 유체이탈 쿨타임
        // [FoldoutGroup("Movement"), ShowIf("dodgeEnabled")] public float ghostMoveSpeed = 5f; // 유체이탈 이동속도
        [FoldoutGroup("Movement"), ShowIf("ghostEnabled")] public GameObject ghostPlayer;
        [FoldoutGroup("Movement"), ShowIf("ghostEnabled")] public Image ghostGaugeBar;
        [FoldoutGroup("Movement"), ShowIf("ghostEnabled")] public Color ghostColor;
        [Space]
        [FoldoutGroup("Movement")] public bool interactEnabled = true;
        [Space]
        ...
        [FoldoutGroup("Input"), DisableInEditMode] public bool inputJumpDown = false;                          // jump down input
        ...
        [FoldoutGroup("Input"), DisableInEditMode] public bool inputGhost = false;                              // ghost input
        [FoldoutGroup("Input"), DisableInEditMode] public bool inputGhostStop = false;                              // ghost input
        [FoldoutGroup("Input"), DisableInEditMode] public bool inputInteract = false;                              // ghost input
        ...
        private PixelCharacterInputMouseAndKeyboard inputMouseAndKeyboard;
        ...
        public void Jump()
        {
            ...
            // 기어가거나 구르는 중이면 점프못함
            if (isCrawling || isCrawlEntering || isCrawlExiting || isDodging) return;
        ...
        */
        ...
        public bool IsGrounded
        {
            ...
            private set
            {
                ...
                {
                    currentJumpCount = 0;
                    if(onGliding) SetGliding(false, 1);
                    if(velocity.y < -LAND_EVENT_THRESHOLD) onLand.Invoke(); //ground event
                }
            }
        }
        ...
        public void GroundCheck()
        {
            ...
            if (onGliding && velocity.y <= 0) gravityScale = isGrounded ? 0.0f : glidingGravity;
            else gravityScale = isGrounded ? 0.0f : 1f;
        ...
        private float GhostRestTime
        {
            get { return ghostRestTime; }
            set
            {
                ghostRestTime = Mathf.Clamp(value, 0, ghostHoildingTime);
                if (ghostGaugeBar == null) return;
                ghostGaugeBar.fillAmount = ghostRestTime / ghostHoildingTime;
            }
        }

        public void GhostCheck()
        {
            GhostRestTime = onGhost ? GhostRestTime - Time.deltaTime : GhostRestTime + Time.deltaTime;
            ...
            if (onGhost)
            {
                ...
                if (GhostRestTime <= 0 || Input.GetKeyDown(inputMouseAndKeyboard.ghostKey))
                    GhostStop();
            }
        ...
        private void GhostStop()
        {
            ...
            inputMouseAndKeyboard.InputKeyReset();
            inputMouseAndKeyboard.enabled = true;
        }

        public void GhostStart()
        {
            if (ghostEnabled == false) return;

            if (onGhost == false)
            {
                if (GhostRestTime < ghostCooldown) return; // 단 쿨타임까지 못채우면 중단
                
                onGhost = true;

                currentGhost = PoolManager.Instance.Pop(ghostPlayer);
                inputMouseAndKeyboard.enabled = false;
                inputMouseAndKeyboard.InputKeyReset();
            }
        }
        ...
        private bool onGliding = false;
        private float glidingGravity = 1;
        ...
        public void SetGliding(bool active, float amount)
        {
            onGliding = active;
            if(velocity.y < 0)
            {
                velocity.y = Mathf.Min(velocity.y * amount, 0);
                rb2d.velocity = velocity;
            }

            glidingGravity = amount;
        }
        ...
        private void Awake()
        {
            ...
            inputMouseAndKeyboard = GetComponent<PixelCharacterInputMouseAndKeyboard>();
